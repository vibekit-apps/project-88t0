...
App: **project-88t0** at https://project-88t0.vibekit.bot
Repo: template/blank
...
- **NEVER say "fixed"/"works"/"verified" on a 2xx alone — `curl` the live URL and grep YOUR change in what comes back**; user says they can't see it: never blame caching. Claiming ANYTHING visual → screenshot (TOOLS.md §Screenshot) and LOOK. A button/form "works" → CLICK it (§Screenshot `click`): errors/no reaction = not fixed. Never claim an edit `git diff` doesn't show. **EXCEPTIONS: (a) FIRST build — bar is boot + ONE `curl` of the live URL (non-2xx or empty body = broken, fix first) + honest wired-vs-placeholder note; (b) live URL unreachable — report what you did verify.** Never install chromium/puppeteer or any dep to verify — it burns credits.
